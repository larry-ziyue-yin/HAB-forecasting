...
exit_with_error() {
    ...
    echo $1
    echo
    echo "https://data.ornldaac.earthdata.nasa.gov/protected/daymet/Daymet_Daily_V4R1/data/daymet_v4_daily_na_tmin_2024.nc"
    echo
    exit 1
}
...
prompt_credentials
  detect_app_approval() {
    approved=`curl -s -b "$cookiejar" -c "$cookiejar" -L --max-redirs 5 --netrc-file "$netrc" https://data.ornldaac.earthdata.nasa.gov/protected/daymet/Daymet_Daily_V4R1/data/daymet_v4_daily_na_tmin_2024.nc -w '\n%{http_code}' | tail  -1`
...
setup_auth_curl() {
    ...
    status=$(curl -s -z "$(date)" -w '\n%{http_code}' https://data.ornldaac.earthdata.nasa.gov/protected/daymet/Daymet_Daily_V4R1/data/daymet_v4_daily_na_tmin_2024.nc | tail -1)
